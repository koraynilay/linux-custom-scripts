...
ffmpeg_opts_video+="-f pulse -i "$(pactl get-default-source)"  -ac 1 " #pulseffects_mic  # microphone
#ffmpeg_opts_video+="-f pulse -i alsa_input.usb-0c76_USB_PnP_Audio_Device-00.mono-fallback -ac 1 " #pulseffects_mic  # microphone
...
ffmpeg_opts_video+="-c:v hevc_nvenc -r:v 60 -b:v 10M -bufsize 5M -pix_fmt yuv444p " #pix_fmt yuv444p perché se no il rosso/magenta è sfocato
...
case ${args[$(($OPTIND-1))]} in
	...
	shotf)
		filename="${folder_img}/${filename:=${date}}.${image_ext}"
		read -r W H < <(xrandr -q | grep -Po '(?<=current )[0-9]{4} x [0-9]{4}' | sed 's/x //g') #[1]
		X=0
		Y=0
		ffmpeg_opts_image=${ffmpeg_opts_image/size_to_replace/-s ${W}x${H}}
		ffmpeg_opts_image=${ffmpeg_opts_image/offset_to_replace/+${X},${Y}}
